...
// POST /auth/signup  - Creates a new business in the database
...
router.post("/business/signup", (req, res, next) => {
  ...
  // Use regex to validate the business email format
  const emailRegex = /^[^\s@]+@[^\s@]+\.[^\s@]{2,}$/;
  if (!emailRegex.test(email)) {
    res.status(400).json({ message: "Provide a valid email address." });
    return;
  ...
  // Use regex to validate the business password format
  const passwordRegex = /(?=.*\d)(?=.*[a-z])(?=.*[A-Z]).{6,}/;
  if (!passwordRegex.test(password)) {
    res
      .status(400)
    ...
    // Create the new business in the database
    // We return a pending promise, which allows us to chain another `then`
    return Business.create({ email, password: hashedPassword, name });
  })
...
// Check the business collection if a business with the same email exists
Business.findOne({ email })
.then((foundBusiness) => {
  if (!foundBusiness) {
  ...
  if (passwordCorrect) {
    ...
    const { _id, email, name } = foundBusiness;
...
// Send back the object with business data
// previously set as the token payload
res.status(200).json(req.payload);
});
